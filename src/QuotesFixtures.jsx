...
function RegionCell({value,onSave}){
 ...
 function commit(raw=draft){const q=String(raw||"").trim(),hit=bestMatch(raw);if(hit)onSave(hit);else if(q&&q!==String(value||"").trim()){}setEdit(false)}
 return <td style={{...POS_TD,padding:edit?"1px 2px":POS_TD.padding,fontWeight:500,color:C.tx,position:"relative",overflow:"visible"}} onClick={open}>
   {!edit?value||"":<input ref={ref} value={draft} onClick={e=>e.stopPropagation()} onChange={e=>{setDraft(e.target.value);setActiveIdx(0)}} onBlur={()=>setTimeout(()=>commit(),150)} onKeyDown={e=>{if(e.key==="ArrowDown"){e.preventDefault();setActiveIdx(i=>Math.min(i+1,Math.max(0,matches.length-1)))}else if(e.key==="ArrowUp"){e.preventDefault();setActiveIdx(i=>Math.max(i-1,0))}else if(e.key==="Enter"){e.preventDefault();commit(matches[activeIdx]||draft)}else if(e.key==="Tab"){e.preventDefault();commit(matches[activeIdx]||draft)}else if(e.key==="Escape")setEdit(false)}} style={{width:"100%",height:27,lineHeight:"25px",padding:"0 5px",margin:0,border:"1px solid rgba(58,130,246,.32)",borderRadius:4,outline:"none",boxShadow:"none",background:"rgba(20,39,66,.78)",color:C.tx,fontFamily:"inherit",fontSize:12,fontWeight:500,textTransform:"uppercase",boxSizing:"border-box"}}/>}
  ...
  <div style={POS_WRAP} className="pos-hover-rows">
   ...
   <table style={POS_TABLE}>
    ...
    {shown.map(([k])=>{
      ...
      return <Editable key={k} value={val} align={["from","to"].includes(k)?"center":"left"} bold={k==="charterer"} color={k==="vessel"?C.blue:k==="charterer"?"#79c0ff":k==="qty"?C.amber:k==="freight"?"#a8e6a3":C.tx} onSave={v=>onUpdateC(c.id,k,k==="charterer"||k==="load"||k==="disch"?toTCase(v):k==="qty"?normaliseQty(v):k==="freight"?(fmtFreight(v)||v):v)}/>;
