...
function RegionCell({value,onSave}){
 ...
 function positionPopup(){const r=cellRef.current?.getBoundingClientRect();if(r)setPop({left:r.left,top:r.bottom+1,width:Math.max(180,r.width)});}
 function open(e){e?.stopPropagation();setDraft(value||"");positionPopup();setEdit(true);setTimeout(()=>{positionPopup();ref.current?.focus();ref.current?.select()},0)}
 ...
 return <><td ref={cellRef} style={{...POS_TD,padding:edit?"0 8px":POS_TD.padding,fontWeight:500,color:C.tx,position:"relative"}} onClick={open}>
