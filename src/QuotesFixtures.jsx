...
function RegionCell({value,onSave}){
 const [edit,setEdit]=useState(false),[draft,setDraft]=useState(value||""),[pop,setPop]=useState({left:0,top:0,width:180}),ref=useRef(null),cellRef=useRef(null);
 const matches=REGIONS.filter(r=>!draft||r.toLowerCase().startsWith(draft.toLowerCase())||r.toLowerCase().includes(draft.toLowerCase()));
 function open(){setDraft(value||"");const r=cellRef.current?.getBoundingClientRect();if(r)setPop({left:r.left,top:r.bottom+2,width:Math.max(180,r.width)});setEdit(true);setTimeout(()=>ref.current?.focus(),0)}
 ...
 return <><td ref={cellRef} style={{...POS_TD,padding:edit?"0 8px":POS_TD.padding,fontWeight:500,color:C.tx}} onClick={open}>
 {!edit?value||"":<input ref={ref} value={draft} onChange={e=>setDraft(e.target.value)} onBlur={()=>setTimeout(commit,120)} onKeyDown={e=>{if(e.key==="Enter"||e.key==="Tab"){e.preventDefault();commit()}if(e.key==="Escape")setEdit(false)}} style={{width:"100%",height:20,lineHeight:"18px",padding:"0 6px",margin:0,border:"1px solid rgba(58,130,246,.32)",borderRadius:4,outline:"none",boxShadow:"none",background:"rgba(20,39,66,.78)",color:C.tx,fontFamily:"inherit",fontSize:12,fontWeight:500,textTransform:"uppercase",boxSizing:"border-box"}}/>}
 </td>{edit&&matches.length>0&&<div style={{position:"fixed",left:pop.left,top:pop.top,zIndex:60000,width:pop.width,background:"#071223",border:"1px solid "+C.bd,borderRadius:5,padding:3,boxShadow:"0 8px 25px rgba(0,0,0,.65)",overflow:"visible"}}>{matches.map(r=><div key={r} onMouseDown={e=>{e.preventDefault();onSave(r);setEdit(false)}} style={{padding:"5px 7px",fontSize:11,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap"}}>{r}</div>)}</div>}</>;
}
  ...
  <div style={POS_WRAP}>
   <table style={POS_TABLE}>
    ...
    <tbody>{filtered.slice(0,200).map((c,i)=>{
      ...
      return <tr key={c.id} onMouseEnter={()=>setHoverRowId(c.id)} onMouseLeave={()=>setHoverRowId(null)} style={{background:rowBg,height:32,transition:"background .08s ease"}}><td style={{...POS_TD,textAlign:"center",color:C.faint,padding:"0 2px"}}>[ ]</td>
    {shown.map(([k])=>{
