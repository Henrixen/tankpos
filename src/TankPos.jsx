...
// Module-level fallback — overwritten by window.vesselDB once loaded
const vesselDB = {};
...
function EC({value,color,placeholder,onSave,bold,onTab,onShiftTab,onEnter,...rest}){
  ...
  return(
    <td onClick={start} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} title={value||(placeholder||"Click to edit")}
      style={{padding:"4px 7px",cursor:"text",textTransform:"uppercase",outline:hov?"1px solid rgba(79,195,247,.3)":"1px solid transparent",outlineOffset:"-1px",verticalAlign:"middle",transition:"background .1s",whiteSpace:"nowrap",overflow:"hidden",maxWidth:0}} {...rest}>
...
function DesktopApp({vessels,cargoes,cargoTotal,onUpdateV,onRenameV,onUpdateC,onAddVessels,onAddCargoes,onAddV,onAddC,onDelV,onDelC,hasMore,onLoadMore,onCargoSearch}){
  ...
  const [posFileDaysBack,setPosFileDaysBack]=useState(30);
...
list = list.filter(v => {
  if(!v.fileDate) return true; // keep manual positions without fileDate
...
if(updFilter){
  ...
  {posFileDaysBack===30 ? "Showing all positions" : `${fmtShortDate(new Date(new Date().setDate(new Date().getDate()-posFileDaysBack)))} → ${fmtShortDate(new Date())}`}
</span>
  </div>
...
<span style={{fontSize:11,color:C.dim,width:80,textAlign:"right"}}>
  {posFileDaysBack===0 ? "Today only" : posFileDaysBack===30 ? "All (30d)" : posFileDaysBack+"d back"}
</span>
  </div>
                      ...
                      {filtV.slice(0, posPage*POS_PAGE_SIZE).map((v,i)=>{
                        ...
                        return(
                          <tr key={v.vessel} style={{background:bg,outline:isSel?"1px solid rgba(88,166,255,.2)":"1px solid transparent",cursor:"pointer"}} onClick={()=>setSel(sel===v.vessel?null:v.vessel)}>
                            ...
                            <td style={{...td,textTransform:"uppercase",color:C.dim,whiteSpace:"nowrap",cursor:"default",overflow:"hidden",maxWidth:0}} title={v.built||""}>{v.built||""}</td>
                            <td style={{...td,textTransform:"uppercase",color:C.amber,whiteSpace:"nowrap",overflow:"hidden",maxWidth:0}} title={fmtN(v.dwt)}>{fmtN(v.dwt)}</td>
                            <td style={{...td,textTransform:"uppercase",color:C.dim,whiteSpace:"nowrap",overflow:"hidden",maxWidth:0}} title={v.loa||""}>{v.loa||""}</td>
                            <td style={{...td,textTransform:"uppercase",color:C.dim,whiteSpace:"nowrap",overflow:"hidden",maxWidth:0}} title={v.beam||""}>{v.beam||""}</td>
                            <td style={{...td,textTransform:"uppercase",color:C.dim,whiteSpace:"nowrap",overflow:"hidden",maxWidth:0}} title={fmtN(v.cbm)}>{fmtN(v.cbm)}</td>
                            ...
                            <EC value={v.comment} color={C.dim} placeholder="Comment" onSave={val=>onUpdateV(v.vessel,"comment",val)} data-vid={v.vessel+"-comment"} onTab={()=>{const next=filtV[i+1];if(next)document.querySelector(`[data-vid="${next.vessel}-op"]`)?.click();}} onShiftTab={()=>document.querySelector(`[data-vid="${v.vessel}-port"]`)?.click()} onEnter={()=>{const next=filtV[i+1];if(next)document.querySelector(`[data-vid="${next.vessel}-comment"]`)?.click();}}/>
                             <td style={{...td,textTransform:"uppercase",fontSize:12,color:C.faint,whiteSpace:"nowrap",overflow:"hidden",width:colWidthsV.FileDate||90}}>{v.fileDate?new Date(v.fileDate).toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"}):""}</td>
