...
export default function CalendarTab() {
  ...
  const today = new Date(); today.setHours(0,0,0,0);
  // Start from current month, show 6 months
  ...
  // Build array of {year, month} for NUM_MONTHS months starting from startYear/startMonth
  ...
  function prevPeriod() {
    let m = startMonth - NUM_MONTHS, y = startYear;
    while (m < 0) { m += 12; y--; }
  ...
  function openAdd(dateStr) {
    setEditId(null);
    setForm({ title: "", date: dateStr || todayStr, endDate: "", color: EVENT_COLORS[0], note: "", category: "" });
    setShowForm(true);
  }
  function openEdit(e) {
    setEditId(e.id);
    ...
    <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, padding: "4px 0" }}>
          <button onClick={prevPeriod} style={{ ...btn(false), padding: "3px 10px" }}>‹</button>
          <span style={{ fontSize: 14, fontWeight: 700, color: "rgba(200,220,250,0.85)" }}>
            {MONTH_NAMES[startMonth]} {startYear} — {(() => { let m = startMonth + NUM_MONTHS - 1, y = startYear; while(m>11){m-=12;y++;} return MONTH_NAMES[m]+" "+y; })()}
          </span>
          <button onClick={nextPeriod} style={{ ...btn(false), padding: "3px 10px" }}>›</button>
          <button onClick={() => { setStartMonth(today.getMonth()); setStartYear(today.getFullYear()); }} style={{ ...btn(false) }}>Today</button>
          <span style={{ flex: 1 }} />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {months.map(({ year, month }) => {
            ...
            return (
              <div key={year + "-" + month} style={{ border: "1px solid rgba(58,130,246,0.18)", borderRadius: 7, overflow: "hidden" }}>
                {/* Month header */}
                <div style={{ padding: "5px 10px", background: "rgba(14,22,40,0.98)", borderBottom: "1px solid rgba(58,130,246,0.12)", display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: year === today.getFullYear() && month === today.getMonth() ? "#58a6ff" : "rgba(180,210,250,0.7)" }}>
                    {MONTH_NAMES[month]} {year}
                  </span>
                </div>
                {/* Day headers */}
                <div style={{ display: "grid", gridTemplateColumns: "32px repeat(7, 1fr)", background: "rgba(14,22,40,0.95)", borderBottom: "1px solid rgba(58,130,246,0.10)" }}>
                  <div style={{ padding: "3px 2px", fontSize: 9, color: "rgba(120,160,220,0.3)", textAlign: "center", fontWeight: 700 }}>Wk</div>
                  {DAY_NAMES.map(d => (
                    <div key={d} style={{ padding: "3px 4px", fontSize: 9, fontWeight: 700, color: "rgba(120,160,220,0.45)", textTransform: "uppercase", textAlign: "center" }}>{d}</div>
                  ))}
                </div>
                {/* Weeks */}
                {Array.from({ length: calDays.length / 7 }, (_, wi) => {
                  ...
                  return (
                    <div key={wi} style={{ display: "grid", gridTemplateColumns: "32px repeat(7, 1fr)", borderTop: wi > 0 ? "1px solid rgba(255,255,255,0.035)" : "none" }}>
                      <div style={{ padding: "4px 2px", fontSize: 9, color: "rgba(120,160,220,0.25)", textAlign: "center", background: "rgba(14,22,40,0.5)", display: "flex", alignItems: "flex-start", justifyContent: "center", paddingTop: 5 }}>{wn}</div>
                      {weekDays.map(({ date, cur }) => {
                        ...
                        return (
                          <div key={ds} onClick={() => setSelectedDay(ds === selectedDay ? null : ds)}
                            style={{ minHeight: 58, padding: "4px 4px 3px", background: isSelected ? "rgba(88,166,255,0.10)" : isToday ? "rgba(88,166,255,0.05)" : (isSat || isSun) ? "rgba(255,255,255,0.01)" : "transparent", cursor: "pointer", borderLeft: "1px solid rgba(255,255,255,0.03)", position: "relative" }}>
                            <div style={{ fontSize: 11, fontWeight: isToday ? 700 : 400, color: isToday ? "#58a6ff" : cur ? (isSat || isSun ? "rgba(120,160,220,0.3)" : "rgba(180,200,230,0.55)") : "rgba(120,160,220,0.15)", marginBottom: 2, display: "flex", alignItems: "center", gap: 3 }}>
                              {isToday
                                ? <span style={{ width: 17, height: 17, background: "#58a6ff", borderRadius: "50%", display: "inline-flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 10, fontWeight: 700, flexShrink: 0 }}>{date.getDate()}</span>
                                : date.getDate()}
                            </div>
                            {evs.slice(0, 2).map(e => (
                              <div key={e.id} style={{ fontSize: 9, background: (e.color || "#58a6ff") + "22", border: "1px solid " + (e.color || "#58a6ff") + "45", borderRadius: 2, padding: "1px 3px", marginBottom: 1, color: e.color || "#58a6ff", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", lineHeight: 1.4 }}>
                                {e.title}
                              </div>
                            ))}
                            {evs.length > 2 && <div style={{ fontSize: 8, color: "rgba(120,160,220,0.35)", paddingLeft: 2 }}>+{evs.length - 2}</div>}
                            {cur && isSelected && (
                              <button onClick={e => { e.stopPropagation(); openAdd(ds); }}
                                style={{ position: "absolute", top: 2, right: 2, background: "rgba(88,166,255,0.2)", border: "1px solid rgba(88,166,255,0.4)", borderRadius: 3, color: "#58a6ff", fontSize: 9, cursor: "pointer", padding: "0 3px", lineHeight: "14px" }}>+</button>
                            )}
                          </div>
                        );
                      })}
      ...
      {/* ── RIGHT: Upcoming + form ── */}
      <div style={{ width: 280, flexShrink: 0, display: "flex", flexDirection: "column", gap: 12, position: "sticky", top: 80 }}>
        {/* Event form */}
        {showForm && (
          <div style={{ border: "1px solid rgba(88,166,255,0.35)", borderRadius: 7, overflow: "hidden" }}>
            ...
            <div style={{ padding: 12, background: "rgba(10,18,34,0.95)", display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div>
                ...
                <input value={form.category} onChange={e => setForm(f => ({ ...f, category: e.target.value }))} style={inp} placeholder="Conference, Regulation, Fixture…" />
              </div>
              <div>
                <div style={{ fontSize: 10, color: "rgba(120,160,220,0.5)", marginBottom: 3, textTransform: "uppercase", letterSpacing: "0.07em" }}>Notes</div>
                <textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} style={{ ...inp, minHeight: 54, resize: "vertical", lineHeight: 1.5 }} placeholder="Any additional notes…" />
              </div>
              <div>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {EVENT_COLORS.map(col => (
                    <div key={col} onClick={() => setForm(f => ({ ...f, color: col }))}
                      style={{ width: 16, height: 16, borderRadius: "50%", background: col, cursor: "pointer", border: form.color === col ? "2px solid #fff" : "2px solid transparent" }} />
                  ))}
                </div>
              </div>
              <div style={{ display: "flex", gap: 6, marginTop: 2 }}>
                <button onClick={saveEvent} style={{ ...btn(true), flex: 1, padding: "5px 0", fontSize: 12 }}>{editId ? "Save changes" : "Add event"}</button>
        ...
        <div style={{ border: "1px solid rgba(58,130,246,0.18)", borderRadius: 7, overflow: "hidden" }}>
          ...
          {upcoming.length === 0 && <div style={{ padding: "20px", textAlign: "center", color: "rgba(120,160,220,0.3)", fontSize: 12 }}>No upcoming events</div>}
          {upcoming.map((e, i) => {
              ...
              <div key={e.id} style={{ background: i % 2 === 0 ? "rgba(10,18,34,0.95)" : "rgba(16,28,52,0.85)", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                <div style={{ padding: "6px 10px", display: "flex", alignItems: "flex-start", gap: 8 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "rgba(140,165,205,0.45)", marginTop: 1 }}>{fmtDate(e.date)}{e.endDate && e.endDate !== e.date ? " – " + fmtDate(e.endDate) : ""}</div>
                    {e.category && <div style={{ fontSize: 10, color: "rgba(120,160,220,0.35)" }}>{e.category}</div>}
                    {e.note && isExp && <div style={{ fontSize: 11, color: "rgba(160,185,215,0.5)", marginTop: 3, lineHeight: 1.5 }}>{e.note}</div>}
                  </div>
                  <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3 }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: du === 0 ? "#43e97b" : du <= 7 ? "#faa356" : du <= 30 ? "#58a6ff" : "rgba(120,160,220,0.4)", whiteSpace: "nowrap" }}>
