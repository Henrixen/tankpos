...
// ─── Compact strip rendered in the global tab bar ────────────────────────────
export function IntelVaultStrip({onVaultUpdate}){
  ...
  function parseItem(item){
    ...
    try{const p=JSON.parse(item.extracted||"{}");if(p.tag) return {...item,tag:p.tag,title:p.title,extracted:p.content};}catch(_){}
  ...
  async function ingest(){
    if(!text.trim()&&!img){setStatus({t:"error",m:"Paste text or image."});return;}
    ...
    try{
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          system:`You are a maritime market intelligence extractor. Today is ${today}. Split the input into logical items. Output as JSON array only, no markdown. Each item: {"tag":"TAG","title":"short title max 6 words","content":"full intel text"}. Tags: RATES,FIXTURE,QUOTE,MARKET,FM,RUMOUR,COUNTERPARTY,EVENT,TC,SALE. Output ONLY the JSON array.`,
      ...
      let parsed=[];
      try{const clean=raw.replace(/^```[\w]*/,"").replace(/```$/,"").trim();parsed=JSON.parse(clean);}catch(e){parsed=[{tag:"MARKET",title:"Market intel",content:raw}];}
      const newItems=[];
      for(const item of parsed){
        const lineItem={id:"iv_"+Date.now()+"_"+Math.random().toString(36).slice(2,5),raw:text,extracted:item.content,title:item.title||item.content.slice(0,40),tag:item.tag||"MARKET",addedAt:intelDate?new Date(intelDate).toISOString():new Date().toISOString(),hasImg:!!img};
      ...
      setText("");setImg(null);
      setStatus({t:"success",m:"✓ "+newItems.length+" item"+(newItems.length!==1?"s":"")+" saved"});
  ...
  return(
    <div style={{position:"relative",display:"flex",flexDirection:"column",minWidth:0}}>
      ...
      {/* ── Dropdown vault panel ── */}
      {expanded&&(
        <div style={{
          position:"absolute",top:"calc(100% + 4px)",right:0,
          width:480,maxWidth:"90vw",
          background:C.bg2,border:"1px solid "+C.bd,borderRadius:7,
          zIndex:200,padding:"10px",
          ...
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
            ...
            <span style={{flex:1}}/>
            <button onClick={()=>setExpanded(false)} style={{background:"none",border:"none",color:C.faint,cursor:"pointer",fontSize:13}}>✕</button>
          </div>
          {/* Tag filters */}
          {usedTags.length>0&&(
          ...
          {/* Item list */}
          <div style={{display:"flex",flexDirection:"column",gap:3,maxHeight:320,overflowY:"auto",scrollbarWidth:"thin",scrollbarColor:C.bd2+" transparent"}}>
            {filtered.length===0&&<div style={{fontSize:12,color:C.faint,fontStyle:"italic",padding:"8px 0"}}>Nothing stored yet.</div>}
            {filtered.map(item=>{
              const col=TAG_COLORS[item.tag]||C.dim;
              return(
                <div key={item.id} style={{display:"flex",alignItems:"center",gap:6,background:C.bg3,border:"1px solid "+col+"33",borderRadius:4,padding:"4px 8px"}}>
