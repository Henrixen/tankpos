  ...
  React.useEffect(()=>{
    ...
    const loadLoginFeeds=async()=>{
      try{
        ...
        if(!alive)return;
        // Login/start page: live /api/login-market values always win.
        // Supabase last-bunker-prices is fallback ONLY when the live endpoint
        // did not return a usable bunker price. This prevents the start page
        // from briefly showing today's price and then jumping back to an older
        // persisted Dashboard snapshot.
        let savedBunkers=null;
        try{
          const {data}=await supabase.from("dashboard").select("value").eq("key","last-bunker-prices").maybeSingle();
          savedBunkers=data?.value?(typeof data.value==="string"?JSON.parse(data.value):data.value):null;
        }catch(_){}

        if(mktRes.status==="fulfilled"&&mktRes.value){
          const live=mktRes.value;
          setLoginMarkets(v=>({
            ...v,
            ...live,
