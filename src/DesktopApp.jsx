        ...
        {tab==="cargo"&&(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            ...
            <div style={{display:"flex",gap:10,alignItems:"flex-start",flexDirection:mobile?"column":"row"}}>
              ...
              {/* Filter panel — same style as positions filter panel */}
              {(()=>{
                ...
                const FR=({label,col,children})=>(
                  <div style={{display:"flex",alignItems:"center",gap:6,borderBottom:"1px solid "+C.bd2,paddingBottom:3}}>
                    <div style={{width:60,fontSize:10,fontWeight:700,color:col,textTransform:"uppercase",flexShrink:0}}>{label}</div>
                    <div style={{display:"flex",flexWrap:"wrap",gap:3,flex:1}}>{children}</div>
                  </div>
                );
                return(
                  <div style={{flex:1,display:"flex",flexDirection:"column"}}>
                    <div style={{display:"flex",flexDirection:"column",justifyContent:"space-between",padding:"7px 10px",background:C.bg3,border:"1px solid "+C.bd2,borderRadius:6,boxSizing:"border-box",gap:4}}>
                      <FR label="Charterer" col={C.amber}>
                        {charterers.slice(0,16).map(ch=>(
                          <button key={ch} onClick={()=>setCChartererFilter(v=>v===ch?"":ch)} style={fb(cChartererFilter===ch)}>{ch}</button>
                        ))}
                        {cChartererFilter&&<button onClick={()=>setCChartererFilter("")} style={{...fb(false),color:C.red,borderColor:C.red+"55"}}>✕</button>}
                      </FR>
                      <FR label="Grade" col={C.purple}>
                        ...
                        {cGradeFilter&&<button onClick={()=>setCGradeFilter("")} style={{...fb(false),color:C.red,borderColor:C.red+"55"}}>✕</button>}
                      </FR>
                      <FR label="Month" col="#7dd3fc">
                        ...
                        {(cLaycanMonthFilter||cLaycanYearFilter)&&<button onClick={()=>{setCLaycanMonthFilter("");setCLaycanYearFilter("");}} style={{...fb(false),color:C.red,borderColor:C.red+"55"}}>✕</button>}
                      </FR>
            ...
            <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
              ...
              <input value={cDateFilter} onChange={e=>setCDateFilter(e.target.value)} placeholder="date…"
                style={{width:60,background:C.bg3,border:"1px solid "+C.bd,borderRadius:4,color:C.tx,fontFamily:"inherit",fontSize:11,padding:"2px 6px",outline:"none"}}/>
            </div>
            ...
            {/* Cargo table — same MatrixTable as positions, with resize + tick-all header */}
            {(()=>{
              ...
              const customCargoColumns=cargoColumns.map(col=>{
                const ck=COL_MAP[col.key];
                return{
                  ...col,
                  label: col.key==="select"
                    ? <span style={{fontSize:11,color:allTicked?"#4fc3f7":C.faint,cursor:"pointer",userSelect:"none"}} onClick={e=>{e.stopPropagation();setSelCargoes(allTicked?new Set():new Set(filtC.map(c=>c.id)));}}>{allTicked?"[✓]":"[ ]"}</span>
                    : <span style={{position:"relative",display:"block",paddingRight:ck?6:0}}>
                        {col.label}
                        {ck&&<span onMouseDown={e=>startResizeC(ck,e)} style={{position:"absolute",right:-2,top:0,bottom:0,width:6,cursor:"col-resize",zIndex:2}}/>}
                      </span>
                };
              });
              return(
                    ...
                    renderRow={(f,td,i)=>{
                      ...
                      return(<>
                        ...
                        <td style={{...tdCtr,width:26,padding:"0 2px"}} onClick={e=>e.stopPropagation()}>
                          <button onClick={e=>{e.stopPropagation();setPendingDel({type:"cargo",id:f.id,label:f.vessel||"cargo"});}} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:10,opacity:0.7}}>✕</button>
                        </td>
                      </>);
                    }}
                  />
