...
const tdTxt = {...td2, textAlign:"left", textTransform:"uppercase"};
  ...
  const posColumns = useMemo(() => {
    ...
    if (mobile) {
      return [
        { key: "select", label: selectAllLabel, align: "center", width: 32 },
        { key: "vessel", label: "Vessel", width: 120 },
        { key: "dwt", label: "DWT", align: "right", width: 50 },
        { key: "built", label: "Built", align: "right", width: 50 },
        { key: "openPort", label: "Port", width: 100 },
        { key: "date", label: "Date", align: "center", width: 70 },
      ];
    }
  ...
  }, [mobile, selVessels.size, filtV.length, colWidthsV]);
...
{(()=>{
  ...
  return (
    <>
      {mobile ? (
        // MOBILE: Simplified 6 columns
        <>
          {/* SELECT */}
          <td style={{...tdTxt, textAlign:"center", width:32, padding:"6px 2px"}}
            onClick={e => {
              e.stopPropagation();
              setSelVessels(p => {
                const n = new Set(p);
            ...
            <span style={{color:selVessels.has(v.vessel)?"#4fc3f7":C.faint, fontSize:13, cursor:"pointer", userSelect:"none"}}>
              {selVessels.has(v.vessel) ? "[✓]" : "[ ]"}
            </span>
          </td>
          
          {/* VESSEL */}
          <td style={{...tdTxt, color: ppt ? "#a8e6a3" : "#79c0ff", fontWeight:700, padding:"6px 8px"}}>
            {toTCase(v.vessel)}
          </td>
          
          {/* DWT */}
          <td style={{...tdNum, color: C.dim, padding:"6px 8px"}}>{fmtN(v.dwt)}</td>
          
          {/* BUILT */}
          <td style={{...tdNum, color: C.dim, padding:"6px 8px"}}>{v.built || ""}</td>
          
          {/* OPEN PORT */}
          <td style={{...tdTxt, color: C.amber, padding:"6px 8px"}}>{v.openPort || ""}</td>
          
          {/* DATE */}
          <td style={{...tdTxt, textAlign:"center", color: ppt ? "#a8e6a3" : C.blue, padding:"6px 8px"}}>
            {v.date || ""}
          </td>
        </>
      ) : (
...
/>
                  ...
                  {selV&&(
                    <div style={{
                      ...
                      flexShrink:0,
                      background:C.bg2,
                      border: mobile ? "none" : "1px solid "+C.bd,
                      ...
                      zIndex: mobile ? 1000 : 10,
                      alignSelf:"flex-start",
                      maxHeight: mobile ? "100vh" : "calc(100vh - 70px)",
                      display:"flex",
                      flexDirection:"column"
                    }}>
                      <div style={{padding:"8px 12px",background:C.bg,borderBottom:"1px solid "+C.bd2,display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexShrink:0}}>
                        ...
                        <button onClick={()=>setSel(null)} style={{background:"none",border:"none",color:C.dim,fontSize:mobile?20:14,cursor:"pointer",padding:mobile?"4px 8px":"0"}}>✕</button>
                      </div>
