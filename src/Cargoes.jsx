 ...
 return <div style={{display:"flex",flexDirection:"column",gap:8}}>
  <div style={{display:"flex",gap:10,height:260}}>
   ...
   <div style={{flex:"0 0 25%",minWidth:0,...card,padding:8,display:"flex",flexDirection:"column",gap:8,overflow:"hidden"}}>
    ...
    <div style={{minHeight:0}}><b style={{fontSize:9,color:C.pink}}>TAG</b><div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:3,marginTop:4,maxHeight:112,overflowY:"auto"}}>{tags.map(t=><button key={t} onClick={()=>setTag(x=>x===t?"":t)} style={{...btn(tag===t),textTransform:"uppercase",textAlign:"left",width:"100%",overflow:"hidden",textOverflow:"ellipsis"}}>{String(t).toUpperCase()}</button>)}</div></div>
   </div>
  ...
  <div style={POS_WRAP} className="pos-hover-rows">
   ...
   <table style={POS_TABLE}>
    ...
    <tbody>{pageRows.map((c,i)=>{const rowBg=POS_ROW(i);return <tr key={c.id} style={{background:rowBg,height:32}}>
     ...
     <Editable value={normaliseQty(c.qty)} align="left" color={C.amber} onSave={v=>onUpdateC(c.id,"qty",normaliseQty(v))}/>
     ...
     <Editable value={fmtFreight(c.freight)||c.freight||""} align="left" color="#a8e6a3" onSave={v=>onUpdateC(c.id,"freight",fmtFreight(v)||v)}/>
