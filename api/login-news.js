export default async function handler(req,res){
    ...
    for(const {type,body:x} of chunks){
      ...
      if(pub){
        ...
        if(!Number.isNaN(d.getTime())){
          ts=d.getTime();
          published=d.toLocaleString("en-GB",{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit",timeZone:"Europe/Oslo"});
        }
      }
  ...
  const results=await Promise.all(feeds.map(async f=>{
    try{return parseFeed(await fetchText(f.url),f.name);}
    catch(e){console.warn(`login-news: ${f.name} failed:`,e?.message||e);return [];}
  }));
