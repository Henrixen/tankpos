...
async function fetchHistory(id,symbol){
  const now=Math.floor(Date.now()/1000),from=now-370*86400;
  const url=`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?period1=${from}&period2=${now}&interval=1d&events=history`;
  const r=await fetch(url,{headers:{"user-agent":"Mozilla/5.0"}}); if(!r.ok)throw new Error(`${id} history ${r.status}`);
  const j=await r.json(),x=j?.chart?.result?.[0],ts=x?.timestamp||[],close=x?.indicators?.quote?.[0]?.close||[];
  return ts.map((t,i)=>({date:new Date(t*1000).toISOString().slice(0,10),price:Number(close[i])})).filter(x=>Number.isFinite(x.price));
}
export default async function handler(req,res){
  ...
  res.status(200).json({items,history,updatedAt:new Date().toISOString(),source:"Trading Economics",historySource:"Yahoo Finance daily futures where available"});
}
